# To-do list app with task status toggling
import csv
import datetime
import os
...
# NEW: Streamlined status toggling
def mark_task_status(tasks, index=None, title=None, status=None):
    if index is None and title is None:
        print("Error: Must provide index or title to mark status.")
        return
    if index is not None:
        if not isinstance(index, int) or index < 0 or index >= len(tasks):
            print("Error: Invalid Index.")
            return
        task = tasks[index]
        new_status = status if status else ("done" if task ["status"] == "pending" else "pending")
        task["status"] = new_status
        # NEW: Status update confirmation
        print(f"Task '{task['title']}' marked as {new_status}.")
    else:
        title = title.lower().strip()
        for i, task in enumerate(tasks):
            if task["title"].lower() == title:
...
def main_menu(tasks):
    menu_options = {
        "1": {"label": "Add Task", "action": lambda: add_task(
            ...
            input("Enter status (done, pending): "),
        ...
        "4": {"label": "Update Task", "action": lambda: (
            view_tasks(tasks),
            update_task(
                tasks,
        ...
        "5": {"label": "Mark Task Done/Pending", "action": lambda: (  # NEW: Menu-driven status updates
            view_tasks(tasks),
            mark_task_status(
                tasks,
                index=int(input("Enter task number to mark (1-based): ")) - 1
                if input("Mark by (1) index or (2) title? Enter 1 or 2: ").strip() == "1"
                else None,
                title=input("Enter task title to mark: ") if input("Mark by (1) index or (2) title? Enter 1 or 2: ").strip() == "2" else None,
                status=input("Enter status (done, pending, or press Enter to toggle): ") or None
            )
        )},
        "6": {"label": "Save Tasks", "action": lambda: save_tasks(tasks)},
        "7": {"label": "Quit", "action": lambda: quit_app(tasks)}
    }
