# To-do list app with text file storage
import csv
import datetime
import os
...
def save_tasks(tasks, filename="data/tasks.txt"):
    try:
        # NEW: CSV formatting for text files
        os.makedirs(os.path.dirname(filename) or ".", exist_ok=True)
        with open(filename, "w", newline="") as file:
            writer = csv.writer(file)
            writer.writerow(["title", "due_date", "status", "created", "priority"]) # Header
            for task in tasks:
                writer.writerow({
                    task["title"],
                    task["due_date"],
                    task["status"],
                    task["created"],
                    task.get("priority", "") # NEW: Handle optional priority field
                })
...
def load_tasks(filename="data/tasks.txt"):
    # NEW: Parsing text to dictionaries
    tasks = []
    try:
        ...
        with open (filename, "r", newline="") as file:
            ...
            for row in reader:
                ...
                if row ["priority"]: # New: Handle optional priority field
